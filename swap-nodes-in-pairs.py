...
class ListNode:
    ...
    def __repr__(self):     # Return a string containing a printable representation of an object. 
        if self:
            return "{} -> {}".format(self.val, self.next)   # self.next is the address of the next node, and then take the value out and find_root the next node, etc till the end (None)
