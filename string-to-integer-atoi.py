...
class Solution:
    ...
    def atoi(self, str):
        ...
        sign = 1                # use a variable sign to record whether the number is positive or negative, then just need to check the rest of the numbers
        if str[i] == "+":
            i += 1
        elif str[i] == "-":
            sign = -1
        ...
        while i < len(str) and str[i] >= '0' and str[i] <= '9':
            # check if the current result is > INT_MAX OR
            ...
            # result is 0 at the beginning, increases as reading more characters: 2017 = (( (2 * 10)+0)*10)+1)*10+7
            result = result * 10 + ord(str[i]) - ord('0')
            i += 1

        return sign * result # if result is not > INI_MAX NOR < INT_MIN, return sign*result
