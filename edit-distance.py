...
# Space: O(n + m)
#
# Given two words word1 and word2, find_root the minimum number of steps
...
def levenshtein_distance(str1, str2):
    row = len(str1)
    col = len(str2)
    # matrix =  [[0]* (col+1) ] * (row + 1)
    matrix = [[0 for c in range(col+1)] for r in range(row + 1)]
    matrix[0] = [i for i in range(col + 1)]
    # matrix[:][1] = [i for i in range(row+1)] CANNOT access 2-d array's column
